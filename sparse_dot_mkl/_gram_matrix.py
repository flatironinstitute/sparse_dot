...
_mkl_sp_transpose_ops = {
    (False, False): SPARSE_OPERATION_NON_TRANSPOSE,
...
def _gram_matrix_sparse_to_dense(
    matrix_a,
    aat=False,
    scalar=1.0,
    out=None,
    ...
    # This fixes a specific bug in mkl_sparse_d_syrkd which returns a full
...
# Dict keyed by ('double_precision_bool', 'complex_bool')
_mkl_blas_skry_funcs = {
    (False, False): MKL._cblas_ssyrk,
...
# Dict keyed by ('transpose_bool', 'complex_bool')
_mkl_cblas_transpose_ops = {
    (False, False): CBLAS_NO_TRANS,
...
def _gram_matrix_dense_to_dense(
    matrix_a, aat=False, scalar=1.0, out=None, out_scalar=None
):
    """
    ...
    # Allocate an array for outputs and set functions and types
    output_arr = _out_matrix(
        (n, n),
        out_dtype,
...
def _gram_matrix(
    ...
    :param matrix: Sparse matrix (CSR format is required but will convert
        if cast is True)
    ...
    if _empty_output_check(matrix, matrix):
        debug_print(
            "Skipping multiplication because AT (dot) A must yield an empty "
            "matrix"
        )
        output_shape = (
    ...
    if _sps.issparse(matrix) and not (is_csr(matrix) or is_csc(matrix)):
        ...
        return _gram_matrix_sparse_to_dense(
            matrix,
            aat=transpose,
            out=out,
            out_scalar=out_scalar
