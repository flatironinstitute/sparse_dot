from sparse_dot_mkl._mkl_interface import (SPARSE_DIAG_NON_UNIT, SPARSE_MATRIX_TYPE_SYMMETRIC, SPARSE_FILL_MODE_FULL)
from sparse_dot_mkl._mkl_interface import MKL
from sparse_dot_mkl._sparse_vector import _sparse_dense_vector_mult
from sparse_dot_mkl.linalg._iss import _MKL_ISS, MKLIterativeSparseSolver, _create_empty_temp_arrays
    ...
    def solve(self):
        ...
        for return_value in self:
            ...
            if self.current_iter > self.max_iter:
                ...
                self.final_code = self.current_iter
                break
...
def gmres(A, b, x0=None, tol=1e-05, restart=None, maxiter=None, M=None, callback=None, restrt=None, atol=None, callback_type=None):
    """
    ...
    :param restrt: DEPRECATED - use restart instead, defaults to None
    :type restrt: None, optional
    ...
    if restrt is not None:
        raise ValueError("restrt is deprecated; use restart instead")
    ...
    with FGMRESIterativeSparseSolver(A, b, x=x0, max_iter=maxiter, a_tol=atol, r_tol=tol) as gmres_solver:
