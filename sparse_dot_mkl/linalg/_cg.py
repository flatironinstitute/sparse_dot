    ...
    def solve(self):
        ...
        for return_value in self:
            ...
            if self.current_iter > self.max_iter:
                _msg = "CG solution did not converge in {n} iterations".format(n=self.current_iter)
...
def cg(A, b, x0=None, tol=1e-05, maxiter=None, M=None, callback=None, atol=None):
    ...
    with CGIterativeSparseSolver(A, b, x=x0, verbose=False, max_iter=maxiter, a_tol=atol, r_tol=tol) as cg_solver:
        cg_solver.set_sparse_matrix_descr(SPARSE_MATRIX_TYPE_SYMMETRIC, SPARSE_FILL_MODE_FULL, SPARSE_DIAG_NON_UNIT)
